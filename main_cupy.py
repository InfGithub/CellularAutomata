...
class Rulizer:
    ...
    def __init__(self, rule: int, dim: int, default: cp.ndarray = None):
        """
        初始化方法。
        ...
        if default is None:
            ...
            rule_repr_array: cp.ndarray = cp.frombuffer(rule_repr, dtype=cp.uint8) - 48

            self.rule_space: cp.ndarray = rule_repr_array.reshape([2] * self.pwvalue)
        else:
            self.rule_space: cp.ndarray = default
    ...
    def rule_space_complement(self):
        ...
        self.rule_space = cp.flip(1 - self.rule_space)
...
class Metatizer:
    ...
    def __init__(self, size: list[int]):
        ...
        self.cell: cp.ndarray = cp.zeros(size, dtype=cp.uint8)
    ...
    def metertion(self, rulizer: Rulizer):
        ...
        padded: cp.ndarray = cp.pad(self.cell, 1, mode="wrap")
        windows: cp.ndarray = as_strided(padded, 
                           shape=self.cell.shape + (3,) * self.ndim,
                           strides=padded.strides * 2)

        flat_windows: cp.ndarray = windows.reshape(-1, 3**self.ndim)

        self.cell.ravel()[:] = rulizer.rule_space[tuple(flat_windows.T)]

    def multiteration(self, rulizer: Rulizer, times: int, steps: int = 16, saving: bool = True) -> cp.ndarray:
        """
        多重迭代方法。
        ...
        """
        if saving:
            result: cp.ndarray = cp.zeros((times + 1,) + self.cell.shape, dtype=cp.uint8)
...
def main():
    ...
    log(f"""迭代完毕，用时 {time() - start_time:.4f} s，1/0：{cp.sum(result == 1) /
    result.size * 100:.4f} % / {cp.sum(result == 0) /
